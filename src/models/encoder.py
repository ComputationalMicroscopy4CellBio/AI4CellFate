...
class Encoder: # Encoder for the VAE
    def __init__(self, img_shape, latent_dim, num_classes, gaussian_noise_std):
        self.img_shape = img_shape
        self.latent_dim = latent_dim
        self.num_classes = num_classes
        self.gaussian_noise_std = gaussian_noise_std
        self.model, self.z_mean, self.z_log_var = self.build_encoder()
    ...
    def build_encoder(self):
        ...
        # Output two values for latent space
        z_mean = SpectralNormalization(Dense(self.latent_dim, name="z_mean"))(X)
        z_log_var = SpectralNormalization(Dense(self.latent_dim, name="z_log_var"))(X)
        ...
        encoder_model = Model(enc_input, [z_mean, z_log_var, z], name='vae_encoder')
